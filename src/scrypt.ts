...
const buttons: NodeListOf<HTMLButtonElement> = document.querySelectorAll<HTMLButtonElement>('.btn-number, .btn-operator, .btn-equal');

//Recorre la lista de botones
buttons.forEach(buttons =>{
  //Por el evento de clic regista el id del elemento.
    buttons.addEventListener('click', () => {
        const buttonId: string = buttons.id;
        //Pasa el display a vacio
        if (buttons.id == "clear") {
            calculos = "";
            //Elimina el ultimo caracter de pantalla.
          } else if (buttons.id == "backspace") {
            let string: string = calculos.toString();
